...
def get_reservations(module, ec2, vpc, tags=None, state=None, zone=None):
    ...
    vpc_subnet_id = vpc_subnets[0]['subnet']['id']
...
def await_spot_requests(module, ec2, spot_requests, count):
    ...
    spot_req_ids = []
    for spot_req in spot_requests:
        spot_req_ids.append(spot_req.id)
    ...
    try:
            ...
            for sirb in spot_requests:
                ...
                for sir in reqs:
                    ...
                    if sir.instance_id is not None:
                        ...
                        raise ValueError("Spot instance request %s failed with status %s and fault %s:%s" % (
                        ...
                        raise ValueError("Spot instance request %s was cancelled before it could be fulfilled." % sir.id)
                        ...
                        if sir.status.code == 'instance-terminated-by-user':
                            ...
                            raise ValueError(spot_msg % (sir.id, sir.status.code, sir.fault.code, sir.fault.message))
        ...
        ec2.cancel_spot_instance_requests(spot_req_ids)
        raise e

    ec2.cancel_spot_instance_requests(spot_req_ids)
    raise RuntimeError("Waited too long. Try different subnet")
        ...
        if not checkmode:
            ...
            for vpc_subnet in vpc_subnets:
                try:
                    ...
                    (instance_dict_array, changed_instance_ids, changed) = create_instances(module, ec2, vpc, vpc_subnet_id=vpc_subnet_id, override_count=to_create)
                    if instance_dict_array:
                        spot_launched=True
                        break
                except Exception as e:

                    module.log("Failed to create spot instances in subnet {}. Failed with {}.".format(vpc_subnet, str(e)))

            # Fall back to on-demand instances
            if not spot_launched:
                del module.params['spot_price']
                vpc_subnet_id = vpc_subnets[0]['subnet']['id']
                (instance_dict_array, changed_instance_ids, changed) = create_instances(module, ec2, vpc, vpc_subnet_id=vpc_subnet_id,
                                                                                        override_count=to_create)
...
def create_instances(module, ec2, vpc, vpc_subnet_id=None, override_count=None):
    """
    ...
    if count_remaining == 0:
        ...
        try:
            ...
            if not spot_price:
                ...
                if private_ip:  #spot req section
                    module.fail_json(
