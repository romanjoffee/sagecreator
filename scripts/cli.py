...
def callb(ctx, param, value):
    print(ctx)


@cli.command(help="Configure infrastructure settings")
@click.option('--pkey', prompt=True, callback=callb, expose_value=False, is_eager=True)
@click.option('--key_pair', callback=callb)
def tes(pkey):
    print(pkey)
...
# @click.option('--version', callback=print_version, expose_value=False, is_eager=True)
